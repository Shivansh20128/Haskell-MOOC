...
mapM2 operation (elem1 : list1) (elem2 : list2) = do
  oth <- mapM2 operation list1 list2
  val <- operation elem1 elem2
  return (val : oth)
------------------------------------------------------------------------------
-- Ex 3: Finding paths.
--
...
visit maze place = do s <- get
                      unless (place `elem` s) $ do modify (place :)
                                                   case lookup place maze of
                                                        Just x -> forM_ x (visit maze)
                                                        _ -> return ()
...
path maze place2 place1 = place1 `elem` execState (visit maze place2) []
...
findSum2 ks ns = do num1 <- ks
                    num2 <- ks
                    let temp_sum = num1 + num2
                    ([(num1,num2,temp_sum) | temp_sum `elem` ns])
...
allSums :: [Int] -> [Int]
allSums xs = [sum elem | elem <- subsequences xs]
...
f1 k acc x = if (x+acc)<=k then Just (x+acc) else Nothing
...
f2 acc x = do xs <- get
              if x `elem` xs then return acc else do modify (x :)
                                                     return (acc + x)
...
instance Monad Result where
  ...
  return :: a -> Result a
  return answer = MkResult answer
  (>>=) :: Result a -> (a -> Result b) -> Result b
  NoResult >>= _ = NoResult
  MkResult result >>= func = func result
  Failure wrong_answer  >>= _ = Failure wrong_answer
...
helperInstance :: (a, Int, [String]) -> [String] -> (a, Int, [String])
helperInstance (old_state, new, myInfo) old_info = (old_state, new, old_info ++ myInfo)
...
instance Functor SL where
  -- implement fmap
  fmap :: (a -> b) -> SL a -> SL b
  fmap func (SL parameter) = SL $ \temp ->
    let (value, temp_value_variable, output) = parameter temp
    in (func value, temp_value_variable, output)
...
instance Monad SL where
  -- implement return and >>=
  return answer = SL (answer,, [])
  operation >>= function = SL run where run out = let (temp_sol, manufactured_thing, printed_info) = runSL operation out
                                                      second_operation = function temp_sol
                                              in helperInstance (runSL second_operation manufactured_thing) printed_info
...
mkCounter = do counter <- newIORef 0
               let inc = atomicModifyIORef counter (\t -> (t + 1, ()))
