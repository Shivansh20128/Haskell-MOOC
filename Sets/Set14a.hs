...
greetText greeting = if T.length(greeting)<=15 then T.pack "Hello, " <> greeting <> T.pack"!" else T.pack"Hello, " <> T.take 15 greeting <> T.pack"...!"
------------------------------------------------------------------------------
...
shout :: T.Text -> T.Text
shout greetings = my_helper (T.words greetings) 0 

my_helper :: Integral t => [T.Text] -> t -> T.Text
my_helper [word] num= if even num then T.map toUpper word else word
my_helper [] _ = mempty
my_helper (word : greetings) y = if odd y then word <> T.pack " " <> my_helper greetings 0 else T.map toUpper word <> T.pack " " <> my_helper greetings 1
------------------------------------------------------------------------------
...
takeStrict lazyText numCharacters = TL.toStrict ((TL.take lazyText numCharacters))
...
byteRange byteRange = if B.length byteRange /= 0 then answer else 0 where answer = B.maximum byteRange - B.minimum byteRange
...
countUtf8Chars byteString = case decodeUtf8' byteString of Left _ -> Nothing
                                                           Right byteString -> Just (T.length (byteString))
...
pingpong stringEncoded=  BL.cycle (BL.append (BL.fromStrict stringEncoded) (BL.reverse (BL.fromStrict stringEncoded)))
