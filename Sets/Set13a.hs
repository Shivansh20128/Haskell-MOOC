...
split string = if ' ' `elem` string then Just (takeWhile (/=' ') string, tail $ dropWhile (/=' ') string) else Nothing
...
checkNumber (str1, str2) = interpret (isValid str1 >> isValid str2) where interpret Nothing = Nothing
                                                                          interpret _ = return (str1, str2)
isValid :: Foldable t => t Char -> Maybe (t Char)
isValid str = if any isDigit str then Nothing else return str
-- checkNumber (a, b) = interpret (check a >> check b)
--   where
--     check x = if any isDigit x then Nothing else return x
--     interpret Nothing = Nothing
--     interpret _ = return (a, b)
...
winner scores player1 player2 = do lookup player1 scores ?> \score1 -> lookup player2 scores ?> \score2 -> helperWinner score1 score2 player1 player2

helperWinner :: (Ord a1, Monad m) => a1 -> a1 -> a2 -> a2 -> m a2
helperWinner x y p1 p2 = if x >= y then return p1 else return p2
------------------------------------------------------------------------------
...
selectSum xs is = mapM (helperSelectSum xs) is >>= \x -> return (sum x)

helperSelectSum :: [a] -> Int -> Maybe a
helperSelectSum array i = if i<0 || i>length array-1 then Nothing else return (array!!i)
------------------------------------------------------------------------------
...
countAndLog log [] = return 0
countAndLog log (elem : list) = if not (log elem) then countAndLog log list
                                else do msg (show elem)
                                        res <- countAndLog log list
                                        return (res+1)
...
balance accountName=BankOp helperBalance where helperBalance (Bank b)=(Map.findWithDefault 0 accountName b,Bank b)
...
update :: State Int ()
update=get >>= \answer->put (answer*2+1)
...
paren xs = do n <- get
              if n == -1 then return () else modify (\n -> n + if xs == '(' then 1 else -1)
...
count e = do xs <- get
             case lookup e xs of
              Just n -> modify (map increase)
              Nothing -> put (xs ++ [(e, 1)])
             where
              increase t@(val1, val2) = if val1/=e then t else (val1,val2+1)
...
occurrences list = length <$> (mapM_ count list >> get)