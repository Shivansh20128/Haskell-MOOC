...
helperInputInOutput :: (Eq a) => [(a,Int)] -> a -> Property
helperInputInOutput list element=True===isValid (lookup element list) where isValid x = case x of (Just x) -> True
                                                                                                  Nothing  -> False
------------------------------------------------------------------------------
...
inputInOutput input output = forAll (elements input) (helperInputInOutput output)
...
helperOutputInInput :: (Eq a) => [a] -> (a, Int) -> Property
helperOutputInInput list element = snd element===length (filter (== fst element) list)

outputInInput :: (Show a, Eq a) => [a] -> [(a,Int)] -> Property
outputInInput input output = forAll (elements output) (helperOutputInInput input)
...
frequenciesProp freq (NonEmpty input) = conjoin (map (\value -> value input (freq input)) [sumIsLength, inputInOutput, outputInInput])
