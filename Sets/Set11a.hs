...
readWords 0=return []
readWords n=do line<-getLine
               words<-readWords (n-1)
               return $ sort (line:words)
...
readUntil f = do line <- getLine
                 if f line then return [] else do value <- readUntil f
                                                  return (line:value)
...
countdownPrint n = if n==0 then do print 0
                                   return ()
                    else do print n
                            countdownPrint (n - 1)
------------------------------------------------------------------------------
...
isums :: Int -> IO Int
isums n = if n==0 then return 0 else helperIsums 0 n

helperIsums :: Int -> Int -> IO Int
helperIsums n 0 = return n
helperIsums val n = do i <- readLn
                       print (i+val)
                       helperIsums (i+val) (n-1)
------------------------------------------------------------------------------
...
while :: IO Bool -> IO () -> IO ()
while cond op = do myCond <- cond
                   when myCond op
                   when myCond (while cond op)
