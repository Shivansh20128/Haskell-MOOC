...
velocity (Distance dis) (Time time) = Velocity (dis / time)

-- travel computes a distance given a velocity and a time
travel :: Velocity -> Time -> Distance
travel (Velocity vel) (Time time) = Distance (vel * time)
...
member val (Set list) = val `elem` list

-- add a member to a set
add :: (Ord a) => a -> Set a -> Set a
add val (Set list) = if member val (Set list)
                   then Set list
                   else Set (takeWhile (<val) list ++ [val] ++ dropWhile (<=val) list)
...
data State = Start | Add | Sweet | Dough | Batter | Mixed | Finished | Error
  deriving (Eq,Show)
...
reverseNonEmpty (ele :| []) = ele:|[]
reverseNonEmpty (ele :| list) = last (ele:list) :| tail (reverse (ele:list))
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  ...
  (Set s1) <> (Set s2) = Set ((sort . nub) $ s1++s2)
...
passwordAllowed str (MinimumLength val) = length str >= val
passwordAllowed str (ContainsSome val) = not $ dropNot val str
passwordAllowed str (DoesNotContain val) = dropNot val str

dropNot :: String -> String -> Bool
dropNot val str = null $ dropWhile (`notElem` val) str
------------------------------------------------------------------------------
...
evaluate (Literal num) = num
evaluate (Operation op num1 num2) = if op == "+" then (+) (evaluate num1) (evaluate num2) else (*) (evaluate num1) (evaluate num2)


render :: Arithmetic -> String
render (Literal num) = show num
render (Operation op num1 num2) = "(" ++ render num1 ++ op ++ render num2 ++ ")"
