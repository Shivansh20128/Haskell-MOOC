...
prettyPrint val_1 val_2 = helperPrettyPrint 1 1 val_1 val_2 ""
    where helperPrettyPrint p q val_1 val_2 str
            | q==val_1+1 = str ++ "\n" ++ helperPrettyPrint (p+1) 1 val_1 val_2 str
            | p==(val_1+1) = str
            | (p,q) `elem` val_2 = str++"Q" ++ helperPrettyPrint p (q+1) val_1 val_2 str
            | otherwise = str++"."++ helperPrettyPrint p (q+1) val_1 val_2 str
...
danger point = any (\a -> sameCol point a || sameRow point a || sameDiag point a || sameAntidiag point a)
...
prettyPrint2 val_1 val_2 = helperPrettyPrint2 1 1 val_1 val_2 ""
  where
    helperPrettyPrint2 p q val_1 val_2 str
      | q == val_1 + 1 = str ++ "\n" ++ helperPrettyPrint2 (p + 1) 1 val_1 val_2 str
      | p == val_1 + 1 = str
      | danger (p, q) val_2 && notElem (p, q) val_2 = str ++ "#" ++ helperPrettyPrint2 p (q + 1) val_1 val_2 str
      | otherwise = if (p, q) `elem` val_2 then str ++ "Q" ++ helperPrettyPrint2 p (q + 1) val_1 val_2 str else str ++ "." ++ helperPrettyPrint2 p (q + 1) val_1 val_2 str
...
fixFirst n [] = Just []
fixFirst n [s] =  if helperGC s > n || helperGR s > n then Nothing else Just [s]
fixFirst n val@(t:s) = if helperGC t > n || helperGR t > n then Nothing else if not (danger t s) then Just val else helperFunc (nextCol t) where helperFunc a
                                                                                                                                                    | helperGC a > n  = Nothing
                                                                                                                                                    | not (danger a s) = Just (a: s)
                                                                                                                                                    | otherwise = helperFunc (nextCol a)
--------------------------------------------------------------------------------
...
step _ [] = []
step num list = case fixFirst num list of
  Nothing -> backtrack list
  Just p -> continue p
--------------------------------------------------------------------------------
...
finish n [] = []
finish n way = if length way==n+1 then tail way else finish n (step n way)

solve :: Size -> Stack
solve n = finish n [(1, 1)]

greet :: Int -> Int -> Stack -> IO ()
greet 0 _ s = print s
greet num p val = do
    greet (num - 1) p (step p val)
    print (step p val)
