...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if m>n then drop (m-n) xs else xs where m=length xs 
...
substring i j s = take (j-i) m where m = drop i s
...
palindromify s = if reverse s == s then s else palindromify (tail m) where m = init s
...
safeIndex xs i = if i>=0 && i<n then Just (xs !! i) else Nothing where n = length xs
...
eitherDiv x y = if y /=0 then Right (div x y) else Left (show x ++ "/" ++ show y)
