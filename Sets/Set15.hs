...
sumTwoMaybes Nothing Nothing = Nothing
sumTwoMaybes _ Nothing = Nothing
sumTwoMaybes Nothing _ = Nothing
sumTwoMaybes num1 num2 = (+) <$> num2 <*> num1
...
statements list1 list2 = (++) <$> ([(++" is not "), (++" is ")] <*> list1) <*> list2
------------------------------------------------------------------------------
...
helperCalculator :: Num a => String -> Maybe (a -> a)
helperCalculator "negate" = Just negate
helperCalculator "double" = Just (2*)
helperCalculator _ = Nothing

calculator :: String -> String -> Maybe Int
calculator myOperation val = helperCalculator myOperation <*> readMaybe val
...
validateDiv num1 num2 = check (num2/=0) "Division by zero!" (num1 `div` num2)
------------------------------------------------------------------------------
...
twoPersons :: Applicative f =>
  ...
  -> f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = (\n1 a1 e1 n2 a2 e2 -> [Person n1 a1 e1, Person n2 a2 e2]) <$> name1 <*> age1 <*> employed1 <*> name2 <*> age2 <*> employed2
------------------------------------------------------------------------------
...
instance Functor Priced where
  fmap myFunctor (Priced cost value) = Priced cost (myFunctor value)

instance Applicative Priced where
  pure :: a -> Priced a
  liftA2 :: (a -> b -> c) -> Priced a -> Priced b -> Priced c
  liftA2 myFunctor (Priced cost1 value1) (Priced cost2 value2) = (Priced (cost1+cost2) (myFunctor value1 value2))
  pure = Priced 0
...
f <#> x = myLiftA2 (\func1 func2 -> func1 func2) f x
...
myFmap operation operand = myPure operation <#> operand
...
tryAll _ [] = empty
tryAll function (elem:list) = forOne <|> forAll where forOne = function elem
                                                      forAll = tryAll function list
...
instance (Functor f, Functor g) => Functor (Both f g) where
  ...
  fmap functor1 (Both functor2) = Both ((fmap.fmap) functor1 functor2)
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  liftA2 functor (Both operand1) (Both operand2) = Both ((liftA2.liftA2) functor operand1 operand2)
  pure :: (Applicative f, Applicative g) => a -> Both f g a
  pure number = Both ((pure . pure) number)
