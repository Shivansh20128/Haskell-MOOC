...
oddFactorial x = if even x then facto (x-1) else facto x
...
myGcd 0 k = k
myGcd k 0 = k
myGcd m n = if m>n then myGcd (m-n) n else myGcd m (n-m)
...
leftpad str n = if length str<n then space ++ str else str where space = spacefunc (n- length str)

spacefunc:: Int -> String
spacefunc 0 = ""
spacefunc c = " " ++ spacefunc (c-1)
...
countdown n = "Ready! " ++ numLoop n ++ "Liftoff!"

numLoop :: Integer -> String
numLoop 0 = ""
numLoop c = show c ++ "... " ++ numLoop (c-1)
