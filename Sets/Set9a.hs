...
workload nExercises hoursPerExercise = if (nExercises * hoursPerExercise)>100 then "Holy moly!" else if (nExercises * hoursPerExercise)<10 then "Piece of cake!" else "Ok." 
...
countValid note_list = length (filter check note_list)
check note=((note!!3)==(note!!5)) || ((note!!2)==(note!!4))
------------------------------------------------------------------------------
...
repeated [_] = Nothing
repeated (num:nums)
  | head nums==num = Just num
  | otherwise = repeated nums
...
sumSuccess list = case length val of 0-> Left "no data"
                                     n-> Right (sum $ map sum val)
  where val = filter helper_sumSuccess list

helper_sumSuccess :: Either String Int -> Bool
helper_sumSuccess (Right x) = True
helper_sumSuccess (Left x)  = False
...
isOpen :: Lock -> Bool
isOpen (Lock code x) = x

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open try (Lock code False) = if try == code then Lock code True else Lock code False
open try (Lock code True) = Lock code True
...
changeCode try (Lock code True)=Lock try True
changeCode try (Lock code False)=Lock code False
...
instance Eq Text where
  (==) :: Text -> Text -> Bool
  Text a == Text b = helperInstance a == helperInstance b

helperInstance :: String -> String
helperInstance s = concat (words s)
...
compose [] val = []
compose (a:list_1) list_2 = case lookup (snd a) list_2 of Nothing -> compose list_1 list_2
                                                          Just b -> (fst a, b):compose list_1 list_2
...
permute p q = helperPermute 0 p q

helperPermute :: Int -> Permutation -> [a] -> [a]
helperPermute num p q = case elemIndex num p of Nothing -> []
                                                Just temp -> (q !! temp) : helperPermute (num + 1) p q