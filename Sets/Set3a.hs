...
firstHalf str = take m str where m = if even (length str) then (length str) `div` 2 else (length str) `div` 2 + 1 
...
capitalizeFirst str = case str of (x:rest) -> toUpper x : rest
...
while check update value = if check value /= True then value else while check update (update value) 
------------------------------------------------------------------------------
...
joinToLength x list = [str1++str2| str1 <- list, str2 <- list, length (str1++str2)==x]
