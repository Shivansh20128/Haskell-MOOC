...
appendAll _ [] = return ()
appendAll str (ele : list) = do b <- readIORef str
                                writeIORef str (b ++ ele)
                                appendAll str list
------------------------------------------------------------------------------
...
swapIORefs val1 val2 = do temp_val1 <- readIORef val1
                          temp_val2 <- readIORef val2
                          writeIORef val1 temp_val2
                          writeIORef val2 temp_val1
...
compose op1 op2 c = do temp <- op2 c
                       op1 temp
...
hFetchLines file = do new_list [] where new_list l = do end <- hIsEOF file
                                                        if end then return l
                                                        else do i <- hGetLine file
                                                                new_list(l++[i])
------------------------------------------------------------------------------
...
interact' f state = do input <- getLine
                       let (c, output, newState) = f (input, state)
                       putStrLn output
                       if not c then return newState else interact' f newState