...
combine pic (Picture p1) (Picture p2) = Picture (\pix -> pic (p1 pix) (p2 pix))
...
rectangle x0 y0 w h = Shape shape
  where shape (Coord x y) = (x >= x0 && x < x0 + w) && (y >= y0 && y < y0 + h)
------------------------------------------------------------------------------
...
union (Shape s1) (Shape s2) = Shape (\pix -> s1 pix || s2 pix)

cut :: Shape -> Shape -> Shape
cut (Shape s1) (Shape s2) = Shape (\pix -> s1 pix && not (s2 pix))
------------------------------------------------------------------------------
...
paintSolid color (Shape shape) (Picture pic) = Picture picNew
  where picNew c = if shape c then color else pic c
------------------------------------------------------------------------------
...
paint (Picture pat) (Shape shape) (Picture base) = Picture pic
  where pic c = if shape c then pat c else base c
------------------------------------------------------------------------------
...
instance Transform Flip where
  apply FlipY (Picture pic) = Picture (\(Coord x y) -> pic (Coord x (-y)))
  apply FlipX (Picture pic) = Picture (\(Coord x y) -> pic (Coord (-x) y))
