...
openDatabase nameOfFile = do db <- open nameOfFile
...
deposit connection accountName depositMoney = execute connection depositQuery (accountName, depositMoney)
...
balance cD nameHolder = do myAmount <- query cD balanceQuery [nameHolder] :: IO [[Int]]
                           return (sum (map sum myAmount))
...
parseCommand wayMoney = if T.pack "balance" == head wayMoney then if length wayMoney /= 2 then Nothing else Just (T.pack "c") >> Just (Balance (head (tail wayMoney))) else if T.pack "withdraw"==head wayMoney then if length wayMoney /= 3 then Nothing else Just (T.pack "c") >> parseInt (wayMoney !! 2) >>= \x -> Just (Withdraw (wayMoney !! 1) x) else if T.pack "deposit"==head wayMoney then if length wayMoney /= 3 then Nothing else Just (T.pack "c") >> parseInt (wayMoney !! 2) >>= \x -> Just (Deposit (wayMoney !! 1) x) else Nothing
------------------------------------------------------------------------------
...
perform connection (Just (Balance name)) = do b <- balance connection name
                                              return (T.pack (show b))
perform connection (Just (Withdraw name money)) = do deposit connection name (-money)
                                                     return (T.pack "OK")
perform connection (Just (Deposit name money)) = do deposit connection name money
                                                    return (T.pack "OK")
