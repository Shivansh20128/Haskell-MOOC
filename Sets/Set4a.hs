...
distinct xs = if length xs==length (nub xs) then True else False
...
rangeOf (x:xs) = abs (snd t - fst t) where t = helperRangeOf x x xs

helperRangeOf :: (Ord a, Num a) => a -> a -> [a] -> (a,a)
helperRangeOf m n [] = (m, n)
helperRangeOf m n (x:xs) = helperRangeOf (min x m) (max x n) xs
...
longest [m] = m
longest (m:n:list)
      | length m > length n  = longest (m:list)
      | length m < length n  = longest (n:list)
      | otherwise = if m < n then longest (m:list) else longest (n:list)
...
incrementKey _ [] = []
incrementKey match ((key, val):xs) = if match==key then (key, val+1) : incrementKey match xs else (key, val) : incrementKey match xs
...
freqs [] = Map.empty
freqs xs = Map.fromList [(x, (length . filter (==x)) xs) | x <- nub xs]
...
swap index1 index2 xs = xs // [(index1, xs ! index2), (index2, xs ! index1)]
...
maxIndex arr = fst (head (sortBy helperMaxIndex (assocs arr))) where helperMaxIndex x y = compare (snd y) (snd x)
